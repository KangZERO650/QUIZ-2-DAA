...
> **Genre:** 3D Survival-Chase · **Engine:** Unity (C#) · **Grid:** 32 × 32 Discrete
...
- Tidak ada edge diagonal, sehingga graph bersifat **planar** dan **sparse** dengan derajat maksimum **Δ = 4**.
...
- **Optimal Substructure:** Jika P* adalah jalur terpendek dari node `s` ke node `t` yang melewati node `v`, maka sub-jalur `s → v` dan `v → t` juga merupakan jalur terpendek masing-masing. Properti ini dipenuhi karena bobot edge non-negatif (w = 1 ≥ 0).
- **Greedy Choice Property:** Pada setiap iterasi, Dijkstra memilih node dengan `gCost` minimum dari *open set*. Keputusan greedy ini bersifat **safe** karena semua bobot ≥ 0, menjamin bahwa node yang di-extract tidak akan pernah diperbarui lagi.
- **Guarantee:** Dijkstra menjamin **shortest path** pada graph dengan bobot non-negatif. Karena semua edge memiliki bobot 1 (uniform), algoritma ini ekuivalen secara output dengan BFS, namun diimplementasikan dengan struktur Dijkstra.
...
- Ini bukan optimasi global, melainkan **greedy local decision** yang meningkatkan tekanan pursuit secara non-deterministik.
...
Proyek menggunakan **Component-Based Architecture** mengikuti paradigma Unity — setiap behavior dipisahkan ke `MonoBehaviour` tersendiri. Kode diorganisasikan dalam struktur folder modular:
